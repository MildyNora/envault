...
pub fn cmd_audit(json: bool) -> Result<()> {
    ...
    // Loading the identity both proves human presence again and unwraps the
    // stable HMAC key needed to verify the chain.
    ...
    if entries.is_empty()
        && matches!(
            audit::verify(&home, key.expose_secret().as_bytes(), &entries),
            Integrity::Ok
        )
    {
    ...
    match audit::verify(&home, key.expose_secret().as_bytes(), &entries) {
